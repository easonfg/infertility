...
for (diag in c( 'DOR')){
  ...
  for (col.i in colnames(cytokines.data)){
    cytokines.data[is.na(cytokines.data[,col.i]), col.i] = mean(cytokines.data[,col.i], na.rm = T)
  }
  ...
  fertile.data = bind_cols(infertility.data[, c('Fertile', 'Age', 'Cycle.Date', 'BMI')], cytokines.data)
