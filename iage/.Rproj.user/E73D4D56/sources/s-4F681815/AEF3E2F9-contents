...
print(auc(roc(org.data$Fertile, org.data$iAge)))
### jacobian iAge####
...
for (diag in c('ENDOMETRIOSIS')){
  ...
  fertile.data = bind_cols(infertility.data[, c('Fertile','iAge', 'Age', 'BMI')], cytokines.data)
  ...
  print(auc(roc(fertile.data$Fertile, fertile.data$iAge)))
...
for (diag_i in unique(all.res$diag)){
  ...
  pdf(paste0('DOR/', diag_i, '.pdf'), height = 10, width = 20, onefile = TRUE)
