## use all cycle collection date on DOR diagnosis
...
for (diag in c('DOR')){
  ...
  fertile.data = bind_cols(infertility.data[, c('Fertile', 'CD.of.blood.draw', 'Age', 'BMI')], cytokines.data)
  ...
  #lm.model = lm(as.formula(paste(x, '~Fertile + BMI + Age +CD.of.blood.draw')), data = fertile.data)
