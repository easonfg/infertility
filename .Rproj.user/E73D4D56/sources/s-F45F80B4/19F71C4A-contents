## ## looking to compare IVF, natural, medicated
...
## get rid of OCPs
org.data = org.data[org.data$Cycle.Type != 'OCPs',]
...
fertile.data = bind_cols(infertility.data[, c('Cycle.Date', 'Cycle.Type', 'Age', 'BMI', 'Fertile', 'Infertility.diagnosis')], cytokines.data)
fertile.data
...
timepoint.combos = combn(fertile.data$Cycle.Type %>% unique() %>% sort(), 2)

all.res = data.frame()
for (col.i in 1:ncol(timepoint.combos)) {
  # for (col.i in 6:ncol(timepoint.combos)) {
  sub.fertile.data = fertile.data[fertile.data$Cycle.Type == timepoint.combos[1, col.i]|fertile.data$Cycle.Type == timepoint.combos[2,col.i],]
  
  res = sapply(colnames(sub.fertile.data)[grep('IL8', colnames(sub.fertile.data)):ncol(sub.fertile.data)], function(x){
    # lm.model = lm(as.formula(paste(x, '~Cycle.Type + Age + Fertile')), data = sub.fertile.data)
    # temp.data1 = sub.fertile.data[sub.fertile.data$Cycle.Type == timepoint.combos[1, col.i],c('Cycle.Type', 'Age', 'Fertile', 'Infertility.diagnosis', x)]
    temp.data1 = sub.fertile.data[sub.fertile.data$Cycle.Type == timepoint.combos[1, col.i],c('Cycle.Type', 'Age', 'BMI', 'Fertile', 'Infertility.diagnosis', x)]
    outliers <- boxplot(temp.data1[,x], plot=FALSE)$out
    temp.data1<- temp.data1[!(temp.data1[,x] %in% outliers),]
    
    # temp.data2 = sub.fertile.data[sub.fertile.data$Cycle.Type == timepoint.combos[2, col.i],c('Cycle.Type', 'Age', 'Fertile', 'Infertility.diagnosis', x)]
    temp.data2 = sub.fertile.data[sub.fertile.data$Cycle.Type == timepoint.combos[2, col.i],c('Cycle.Type', 'Age', 'BMI', 'Fertile', 'Infertility.diagnosis', x)]
    outliers <- boxplot(temp.data1[,x], plot=FALSE)$out
    temp.data2<- temp.data2[!(temp.data2[,x] %in% outliers),]
    
    temp.data = bind_rows(temp.data1, temp.data2)
    lm.model = lm(as.formula(paste(x, '~Cycle.Type + Age +BMI + Fertile+ Infertility.diagnosis')), data = temp.data)
    # lm.model = lm(as.formula(paste(x, '~Cycle.Type + Age + Fertile')), data = temp.data)
    # lm.model = lm(as.formula(paste(x, '~Cycle.Type + Age + Infertility.diagnosis')), data = temp.data)
