## ## looking to compare different chunks
## 0-5 M
## 5-13 F
## 14 Ovulation
## 15-28 L high level of estrogen 
...
fertile.data = bind_cols(infertility.data[, c('Cycle.Date', 'Cycle.Name', 'Age', 'BMI', 'Fertile', 'Infertility.diagnosis')], cytokines.data)
fertile.data
...
fertile.data$Cycle.Name = factor(fertile.data$Cycle.Name, levels = c('O', 'M', 'F', 'L'))
fertile.data$Cycle.Name
fertile.data$Infertility.diagnosis = factor(fertile.data$Infertility.diagnosis)
fertile.data$Cycle.Date

timepoint.combos = combn(fertile.data$Cycle.Name %>% unique() %>% sort(), 2)

all.res = data.frame()
for (col.i in 1:ncol(timepoint.combos)) {
# for (col.i in 6:ncol(timepoint.combos)) {
  sub.fertile.data = fertile.data[fertile.data$Cycle.Name == timepoint.combos[1, col.i]|fertile.data$Cycle.Name == timepoint.combos[2,col.i],]
  
  res = sapply(colnames(sub.fertile.data)[grep('IL8', colnames(sub.fertile.data)):ncol(sub.fertile.data)], function(x){
    # lm.model = lm(as.formula(paste(x, '~Cycle.Name + Age + Fertile')), data = sub.fertile.data)
    # temp.data1 = sub.fertile.data[sub.fertile.data$Cycle.Name == timepoint.combos[1, col.i],c('Cycle.Name', 'Age', 'Fertile', 'Infertility.diagnosis', x)]
    temp.data1 = sub.fertile.data[sub.fertile.data$Cycle.Name == timepoint.combos[1, col.i],c('Cycle.Name', 'Age', 'BMI', 'Fertile', 'Infertility.diagnosis', x)]
    outliers <- boxplot(temp.data1[,x], plot=FALSE)$out
    temp.data1<- temp.data1[!(temp.data1[,x] %in% outliers),]
    
    # temp.data2 = sub.fertile.data[sub.fertile.data$Cycle.Name == timepoint.combos[2, col.i],c('Cycle.Name', 'Age', 'Fertile', 'Infertility.diagnosis', x)]
    temp.data2 = sub.fertile.data[sub.fertile.data$Cycle.Name == timepoint.combos[2, col.i],c('Cycle.Name', 'Age', 'BMI', 'Fertile', 'Infertility.diagnosis', x)]
    outliers <- boxplot(temp.data1[,x], plot=FALSE)$out
    temp.data2<- temp.data2[!(temp.data2[,x] %in% outliers),]
    
    temp.data = bind_rows(temp.data1, temp.data2)
    lm.model = lm(as.formula(paste(x, '~Cycle.Name + Age +BMI + Fertile+ Infertility.diagnosis')), data = temp.data)
    # lm.model = lm(as.formula(paste(x, '~Cycle.Name + Age + Fertile')), data = temp.data)
    # lm.model = lm(as.formula(paste(x, '~Cycle.Name + Age + Infertility.diagnosis')), data = temp.data)
...
for (cyto in unique(long.fertile.data$cytokines)){
  ...
  long.fertile.data = long.fertile.data[!(long.fertile.data$cytokines == cyto & !long.fertile.data$Cycle.Name %in% unique.time), ]
  
  # get rid of outliers
  for (time.i in unique.time){
    one.time.one.cyto = long.fertile.data[(long.fertile.data$cytokines == cyto & long.fertile.data$Cycle.Name %in% time.i), ]
    one.time.one.cyto
...
for (cd.i in unique(sig.all.res$timepoint)){
  ...
  sub.plot.data = long.fertile.data[long.fertile.data$cytokines %in% unique.cyto & long.fertile.data$Cycle.Name %in% unique.time.cd, ]
  ...
  for (i in cyto.chunks.cd){
    ...
    print(
      ggboxplot(temp.fertile.data.cd, x = "Cycle.Name", y = 'measurements',
